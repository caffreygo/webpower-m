...
<template>
    <div class="crm-box" @click="$store.state.menuShow = false;">
        ...
        <!-- <div> -->
            <div  class="mainHTML" v-html="contentHtml"></div>
        <!-- </div> -->
...
export default {
    ...
    mounted(){
        this.$ajax({
            method: 'get',
            url: 'http://www.webpowerchina.com/wp-json/wp/v2/posts?slug='+this.$route.query.slug,
            params: {
                
            }
        }).then(res => {
            this.contentTitle = res.data[0].title.rendered;
            this.contentHtml = res.data[0].content.rendered;
        }).then(err => {
